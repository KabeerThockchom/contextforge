...
class TestSQLiteMemoryStore:
    ...
    async def test_clear_memories_before_date(self, sqlite_memory_store):
        ...
        # Add memory
        await sqlite_memory_store.add_memory({"content": "Test memory"})
        
        # Clear memories before future date
        future_date = datetime.now() + timedelta(hours=1)
        await sqlite_memory_store.clear_memories(future_date)
        
        # Should be no memories left
        import sqlite3
        conn = sqlite3.connect(sqlite_memory_store.db_path)
        cursor = conn.execute("SELECT COUNT(*) FROM memories")
        count = cursor.fetchone()[0]
